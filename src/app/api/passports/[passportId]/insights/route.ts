...
import { generatePassportInsightsForUser } from "@/server/passports/review-service";
...
function clientMessage(error: unknown) {
  return error instanceof AppError ? error.message : "Forge could not complete this enhanced review. Please retry.";
}
...
export async function POST(request: Request, context: { params: Promise<{ passportId: string }> }) {
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        void generatePassportInsightsForUser({
          user,
          passportId,
          requestId,
          onProgress: (progress) => controller.enqueue(event("progress", progress)),
        }).then(({ review, cached }) => {
          controller.enqueue(event("complete", { review, cached }));
          controller.close();
        }).catch((error) => {
