...
test.describe("mocked GitHub OAuth and ingestion flow", () => {
  ...
  test("connects GitHub, shows repository metadata, and creates a source-backed Passport", async ({ page }) => {
    await authenticateWithTestSession(page);
    ...
    await expect(picker.getByText("forge-api")).toBeVisible();
    await expect(picker.getByText(/acme · private/i)).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: /Repository history captured/i })).toBeVisible();
    await expect(page.getByText("Reading repository history")).toBeVisible();
    await page.getByRole("button", { name: /Open source boundaries/i }).click();
    await page.getByRole("button", { name: /Open recent pull requests/i }).click();
    await page.getByRole("button", { name: "Open Change Passport for pull request 42" }).click();

    await expect(page.getByLabel("Change Passport verdict")).toContainText("Insufficient Evidence");
